...
export default class NocalhostDocumentProvider
  ...
  private checkSchema(uri: vscode.Uri) {
    ...
    if (!NocalhostDocumentProvider.supportScheme.includes(uri.scheme)) {
      throw new Error(
        `only support scheme: ${NocalhostDocumentProvider.supportScheme}`
      );
    }

    if (!NocalhostDocumentProvider.supportAuthority.includes(authority)) {
      throw new Error(
        `only support authority: ${NocalhostDocumentProvider.supportAuthority}`
      );
    }
  }
}
