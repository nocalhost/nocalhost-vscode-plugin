...
import { PLUGIN_TEMP_DIR, TMP_DEVSTART_IMAGE } from "./constants";
...
function launchDevspace() {
  ...
  const tmpImage = host.getGlobalState(TMP_DEVSTART_IMAGE);
  ...
  if (tmpApp && tmpWorkload && tmpStatusId && tmpResourceType) {
    ...
    vscode.commands.executeCommand(START_DEV_MODE, node, tmpImage);
  }
}
