...
export function transformPossiblyAsync<T>(
  obj: T | Thenable<T>,
  f: (t: T) => true | Thenable<true>
): T | Thenable<T> {
  if (isThenableStrict(obj)) {
...
export function map<T, U>(
  source: vscode.ProviderResult<T[]>,
  f: (t: T) => U
): U[] | vscode.ProviderResult<U[]> {
...
function isThenable<T>(
  r: vscode.ProviderResult<T>
): r is Thenable<T | null | undefined> {
...
async function appendAsync<T>(
  first: Thenable<T[] | null | undefined>,
  ...rest: Thenable<T[]>[]
): Promise<T[]> {
  return appendSyncAsync(await first, ...rest);
}

async function appendSyncAsync<T>(
...
async function transformThenable<T>(
  obj: Thenable<T>,
  f: (t: T) => void
): Promise<T> {
  f(await obj);
...
async function transformPossiblyAsyncThenable<T>(
  obj: Thenable<T>,
  f: (t: T) => true | Thenable<true>
): Promise<T> {
...
async function mapThenable<T, U>(
  obj: Thenable<T[] | null | undefined>,
  f: (t: T) => U
